...
- 2026-06-07: Created `ai-business-project/` as a standalone package for the user's AI implementation business concept. Keep future AI-business materials in that folder unless the user asks to merge them into another site. --tags ai-business,current,static-site

- 2026-06-07: User selected the first AI pilot direction: collecting information from the internet and analyzing it. Added `AI Research Analyst` MVP in `ai-business-project/research-assistant.html` with a playbook in `docs/RESEARCH_ANALYST_PLAYBOOK.md`. --tags ai-business,research-analyst,current

- 2026-06-07: Upgraded `AI Research Analyst` with `ai-business-project/research-server.js`: local Node server, live web search, URL fetching, HTML cleanup, source summaries, reliability scoring, and `/api/research` + `/api/analyze-urls`. Current test server opened at `http://127.0.0.1:5192/research-assistant.html`; default command uses port 5190. --tags ai-business,research-analyst,live-search,current

- 2026-06-08: Current market-facing AI product packaging is `AI Monitor Studio` / `AI-мониторинг ресурсов под ключ`, with offer page `ai-business-project/ai-monitoring-offer.html`. Main packages: pilot 70k RUB, monitoring setup 120k RUB + 25k RUB/month, team contour from 250k RUB. --tags ai-business,positioning,pricing,current

- 2026-06-08: First-sale AI Monitor Studio offer now includes a limited bonus: mini knowledge base up to 10 materials and simple AI consultant in the 70k RUB pilot, with 15k RUB/month support. Current channel plan lives in `docs/GO_TO_MARKET_AI_MONITORING.md`; objections, meeting scenario, and client brief are separate docs. --tags ai-business,first-sale,go-to-market,current

- 2026-06-08: Added broader AI package catalog at `ai-business-project/service-catalog.html` and `docs/SERVICE_CATALOG_AI_PACKAGES.md`. Keep first sales focused on AI monitoring, but present it as one entry in a broader AI workflow implementation line. --tags ai-business,service-catalog,positioning,current

- 2026-06-08: First outreach niche selected: marketing/e-commerce. Outreach assets live in `docs/FIRST_OUTREACH_ECOMMERCE.md`, `docs/ECOMMERCE_DEMO_SCENARIO.md`, `docs/CONTENT_PLAN_ECOMMERCE.md`, and `docs/LEADS_TEMPLATE_ECOMMERCE.csv`. --tags ai-business,outreach,ecommerce,current

- 2026-06-08: Added first sales workspace at `ai-business-project/outreach-dashboard.html`; use it to copy outreach messages, track 50 e-commerce leads in localStorage, and export CSV. --tags ai-business,outreach-dashboard,current

- 2026-06-08: Company-level AI Monitor Studio launch blueprint now lives in `company-launch-map.html` plus docs `COMPANY_LAUNCH_BLUEPRINT.md`, `ROADMAP_180_DAYS.md`, `USER_TASKS_NEXT_14_DAYS.md`, `BUDGET_AND_RESOURCES.md`, `RESOURCE_LINKS.md`, and `MASTER_CHECKLIST.md`. --tags ai-business,company-launch,roadmap,current

- 2026-06-08: Public buyer-facing website for the AI monitoring offer lives in `ai-business-project/sales-site/`. Keep it separate from `outreach-dashboard.html` (internal sales workspace) and `research-assistant.html` (product/demo prototype). --tags ai-business,sales-site,positioning,current

- 2026-06-08: Client-facing product surface lives in `ai-business-project/client-monitoring-app/`. It is the screen to show after a lead asks what they will receive: sources, analysis run, report, recommendations, mini knowledge base, and report copy/export. --tags ai-business,client-app,product,current
...
- 2026-06-10: Svetlo candle site needs a Blog section for SEO: use short practical product explainers and buying guides for candles, aromas, maker materials, packaging, and wholesale/private-label orders. Blog content should help visitors choose products and link into relevant category/calculator pages, not read like generic news. --tags candles,svetlo,seo,blog,content

- 2026-06-12: Полная проверка всех инструментов агента (Искра). Все инструменты работают: agent:status ✅, check:tools ✅ (Blender, Node, npm, Puppeteer, Three.js, внешний 3D-художник), memory:db ✅ (75 записей), agent:evals ✅, agent:context ✅. Выявлена особенность: на этой Windows PowerShell отключено выполнение скриптов (ExecutionPolicy), поэтому npm-команды нужно запускать через `cmd /c "cd /d <путь> && npm run <команда>"`. После исправления ExecutionPolicy команды будут работать напрямую. --tags agent,iskra,checkup,current,infra
