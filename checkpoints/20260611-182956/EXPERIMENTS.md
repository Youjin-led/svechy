...
### 2026-06-08 - AI Monitor Studio public sales site

- Goal: create a clean public site that can be placed on a domain and used in Avito/social/direct outreach.
- Changed: added `ai-business-project/sales-site/` with `index.html`, `styles.css`, `main.js`, `robots.txt`, `sitemap.xml`, and a deployment README; linked it from the project presentation.
- Scope: the site sells the first offer: AI monitoring of selected internet resources, analysis, report with sources, mini knowledge base, 70 000 RUB pilot, and 15 000 RUB/month support.
- Check: `node --check ai-business-project\sales-site\main.js`, `node --check ai-business-project\app.js`, `node --check ai-business-project\research-assistant.js`, `node --check ai-business-project\outreach-dashboard.js`, and `npm.cmd run qa` passed.
- Browser smoke: `node tools\sales_site_smoke.js` passed after approved browser launch. Desktop/mobile screenshots saved as `ai-business-project/sales-site-desktop.png` and `ai-business-project/sales-site-mobile.png`; no console errors, no horizontal overflow, price/form/CTA/FAQ present.

### 2026-06-08 - AI Monitor Studio client monitoring app

- Goal: create a concrete product screen that can be shown after the public site generates interest.
- Changed: added `ai-business-project/client-monitoring-app/` with a responsive client dashboard: source management, simulated analysis run, report, risks, recommendations, mini knowledge base, AI-consultant answer, and Markdown report copy.
- Product boundary: this is the client-facing product demo; `sales-site/` sells it, `outreach-dashboard.html` helps us find leads, and `research-assistant.html` remains the experimental/live-search prototype.
- Check: pending syntax/QA/browser smoke in this session.

### 2026-06-05 - Candle multipage dropdown navigation cleanup

- Goal: remove the home-page fast-entry section and keep dropdown navigation as the primary way to move through wholesale and maker-goods subcategories.
- Changed: removed `popular-directions` from `index.html`; added the two dropdown menus with 20 sublinks to all pages: home, calculator, wholesale, maker-goods, partners, reviews, and contacts.
- Check: `node --check main.js` passed; `npm.cmd run qa` passed; Puppeteer confirmed every page has 2 dropdowns, 20 dropdown links, no `.popular-directions` / `.direction-grid`, diagnostics present, and no console errors.
- Result: screenshot saved as `candle-multipage-dropdowns-check.png`.

### 2026-06-05 - Candle multipage restructure

- Goal: convert the long scrolling candle site into a cleaner multipage trading site.
- Changed: rebuilt `index.html` as a short home page, added `calculator.html`, `wholesale.html`, `maker-goods.html`, `partners.html`, `reviews.html`, and `contacts.html`; moved the calculator, wholesale categories, maker-goods admin, partners, reviews, and contact content to their own pages.
- JS: made `main.js` page-safe so missing calculator/admin elements do not throw errors, and added `?direction=` support for the calculator page.
- Check: `node --check main.js` passed; `npm.cmd run qa` passed; Puppeteer opened all seven pages, confirmed no console errors, verified `calculator.html?direction=Фитили` preselects `Фитили`, and confirmed diagnostics on every page.
- Result: screenshot saved as `candle-multipage-home-check.png`.

### 2026-06-05 - Candle contextual calculator and sample cards

- Goal: avoid separate calculators under every category card while still letting users calculate from any wholesale or maker-goods category.
- Changed: added one shared `Направление` field in the calculator, `Рассчитать` buttons on 12 wholesale cards and 8 maker-goods cards, JS that fills the direction and scrolls to the calculator, plus two sample product cards shown when no admin products exist.
- Check: `node --check main.js` passed; `npm.cmd run qa` passed; Puppeteer clicked the `Фитили` card, confirmed calculator direction became `Фитили`, found 2 sample cards and 22 contextual calculation buttons, with no console errors.
- Result: screenshot saved as `candle-context-calculator-check.png`.

### 2026-06-05 - Candle ecommerce layout restructure

- Goal: implement the recommended modern trading-site flow: fast category entry, calculator, wholesale catalog, maker goods, partner scenarios, reviews, and contact CTA.
- Changed: added the `directions` section with 6 popular paths, added header CTA, reordered page flow to `directions`, `calculator`, `wholesale`, `maker-goods`, `partners`, `reviews`, `contacts`, updated calculator/partner/review/contact copy, and reordered header/footer navigation around the main commercial paths.
- Check: `node --check main.js` passed; `npm.cmd run qa` passed; Puppeteer confirmed no header overflow, 6 directions, 8 maker categories, 12 wholesale categories, diagnostics present, and no console errors.
- Result: screenshot saved as `candle-ecommerce-structure-pass.png`.

### 2026-06-05 - Candle nav dropdowns

- Goal: turn the `Товары для свечеваров` and `Оптовые продажи` nav tabs into dropdowns with their category subitems.
- Changed: replaced the two plain nav links with accessible `details/summary` dropdowns; added 8 maker-goods subitems and 12 wholesale subitems; styled desktop and mobile dropdown panels in `styles.css`.
- Check: `node --check main.js` passed; `npm.cmd run qa` passed; Puppeteer opened both dropdowns and confirmed item counts 8/12 with no console errors.
- Result: screenshot saved as `candle-dropdowns-check.png`.

### 2026-06-05 - Candle full-power structure pass

- Goal: immediately align the site with the expanded assortment and make the page order match the navigation.
- Changed: updated hero copy to cover candles, aromas, care sets, wholesale, and maker goods; changed hero CTA to wholesale + calculator; moved sections into nav order: `calculator`, `reviews`, `partners`, `maker-goods`, `wholesale`, `contacts`.
- Check: `node --check main.js` passed; `npm.cmd run qa` passed; Puppeteer confirmed nav order, section order, 8 maker categories, 12 wholesale categories, diagnostics present, and no console errors.
- Result: screenshot saved as `candle-full-power-pass.png`.

### 2026-06-04 - Candle expanded tabs without catalog

- Goal: add the requested tabs `Отзывы`, `Партнеры`, `Товары для свечеваров`, and `Оптовые продажи`, while removing the visible `Каталог` tab.
- Changed: replaced `#catalog` with `#maker-goods`, added reviews/partners/wholesale sections, updated header and footer navigation, and kept local card management inside the maker-goods section.
- Check: `node --check main.js` passed; `npm.cmd run qa` passed; Puppeteer confirmed the nav labels, section ids, no visible `Каталог` text, no `#catalog` links, diagnostics present, and no console errors.
- Result: screenshot saved as `candle-tabs-expanded-check.png`.

### 2026-06-03 - Candle site three-section simplification

- Goal: simplify the candle site so the top navigation and page body match only three destinations: calculator, catalog, and contacts.
- Changed: removed standalone category carousel, social strip, shelves, wholesale, about, and separate admin sections; moved the admin card editor inside the catalog section; updated footer and copy away from materials/wholesale structure.
- Check: `node --check main.js` passed; `npm.cmd run qa` passed; Puppeteer confirmed `main > section` is exactly `catalog`, `calculator`, `contacts`, header/footer nav both have `Калькулятор`, `Каталог`, `Контакты`, and console errors are empty.
- Result: screenshot saved as `candle-3-sections-check.png`.

### 2026-06-03 - Candle site MVP QA

- Goal: verify the new candle storefront as the active frontend surface.
- Check: `npm run check:tools`.
- Result: PASS. Node, npm, Puppeteer, Three.js, Blender, and external 3D artist paths are available.
- Check: `node --check main.js`.
- Result: PASS.
- Check: `npm run qa`.
- Result: PASS with `{"status":"PASS","errors":[]}`.
- Check: Puppeteer desktop/mobile visual smoke with screenshots `candle-desktop.png` and `candle-mobile.png`.
- Result: PASS after adding a data favicon and moving mobile hero metrics into normal flow. Console errors: none. Canvas pixels are non-empty. Product cards: 6. Hero/metrics overlap: false.

### 2026-06-03 - Candle calculator redesign QA

- Goal: match the Meliponini-like preorder essence with a simpler, more aesthetic color-palette calculator and messenger/email order routing.
- Check: `node --check main.js`.
- Result: PASS.
- Check: `npm run qa`.
- Result: PASS with `{"status":"PASS","errors":[]}`.
- Check: Puppeteer desktop/mobile visual smoke with screenshots `candle-calc-desktop.png` and `candle-calc-mobile.png`.
- Result: PASS. Console errors: none. Canvas pixels are non-empty. Desktop first viewport shows all 4 order actions: Telegram, WhatsApp, Email, Copy. Mobile first viewport shows the color palette and candle preview before the calculator.

### 2026-06-03 - Candle local admin QA

- Goal: keep public cards empty until the user adds them through a convenient admin surface.
- Check: `node --check main.js`.
- Result: PASS.
- Check: `npm run qa`.
- Result: PASS with `{"status":"PASS","errors":[]}`.
- Check: Puppeteer admin flow: clear `localStorage`, verify empty catalog, add a test card through the admin form, verify public card/list item/storage.
- Result: PASS. Before: 0 cards and empty state visible. After: 1 public card, 1 admin item, and a saved `lumiere-candle-products` entry. Console errors: none. Screenshot: `candle-admin-desktop.png`.
- Check: mobile smoke after admin section.
- Result: PASS. Admin exists, default public catalog has 0 cards and empty state visible. Screenshot: `candle-admin-mobile.png`.

### 2026-06-03 - Candle Meliponini-structure rebuild QA

- Goal: use `meliponini.ru` homepage structure as the site base while applying the red/blue/wine/blush palette and avoiding the reference image/block.
- Check: `node --check main.js`.
- Result: PASS.
- Check: `npm run qa`.
- Result: PASS with `{"status":"PASS","errors":[]}`.
- Check: Puppeteer desktop/mobile structure smoke.
- Result: PASS. Console errors: none. `imgTags`: 0. Main sections: hero, social strip, catalog, calculator, wholesale, about, admin, contacts. Category cards: 4. Calculator total renders. Screenshot: `candle-meliponini-structure-desktop.png`.
- Follow-up: user correctly noted it was still far from the Meliponini structure. Rebuilt the hero into a text-first store hero with promo direction rows, added a category navigation strip, empty product shelves, and separate calculator cards.
- Check: Puppeteer desktop/mobile structure smoke after closer rebuild.
- Result: PASS. Console errors: none. `imgTags`: 0. Promo cards: 4. Category nav links: 8. Calculator cards: 3. Prefilled product cards: 0. Empty catalog visible. Screenshot: `candle-closer-meliponini-desktop.png`.
- Follow-up rebuild: moved closer to Meliponini's actual visual structure with a full-width shop hero, product-like CSS visual on the right, header tools/search/favorites/cart placeholders, and a category carousel immediately under the hero.
- Check: Puppeteer desktop/mobile shop-layout smoke.
- Result: PASS. Console errors: none. `imgTags`: 0. Hero visual present. Category cards: 8. Prefilled product cards: 0. Empty catalog visible. Screenshot: `candle-shop-layout-desktop.png`.
...
### 2026-06-02 - TradeLab twice-daily news schedule

- Goal: refresh news-impact research twice per day without keeping a long-lived process in memory.
- Changed: added `run-tradelab-news.ps1` and scheduled `TradeLab News Impact AM` at 09:05 plus `TradeLab News Impact PM` at 21:05.
- Runtime check: manually started `TradeLab News Impact AM`; `LastTaskResult` was 0 and `TRADELAB_NEWS_SCHEDULE_LOG.md` recorded `Status: PASS`.
- Result: `TRADELAB_NEWS_IMPACT.md` refreshed from public RSS and Binance candles.
- Safety conclusion: scheduled news analysis is research-only and does not affect real-money gate status.

### 2026-06-02 - TradeLab news/market dependency scoring

- Goal: turn news-impact data into explicit dependency hypotheses instead of raw event tables.
- Changed: added `tools/tradelab_news_dependencies.js`, `npm.cmd run tradelab:dependencies`, `TRADELAB_NEWS_DEPENDENCIES.md`, and `tradelab-news-dependencies.json`; scheduled news runner now runs dependency scoring after news refresh.
- Method: group matched news events by symbol/sentiment/horizon, calculate average return, median return, direction agreement, sentiment-return correlation, confidence, and strength (`too-thin`, `weak`, `moderate`, `strong`).
- Check: `node --check tools/tradelab_news_dependencies.js`, `npm.cmd run tradelab:dependencies`, and `npm.cmd run tradelab:doctor` passed.
- Early result: BTC matched news currently shows strong 12h/24h down dependency in a small 23-event sample; ETH all-news 4h/12h up dependency is only moderate and based on 5 events.
- Safety conclusion: dependencies are hypotheses for filtering and investigation only; they must not bypass paper incubation or the real-money gate.

### 2026-06-06 - TradeLab expanded news sources

- Goal: broaden news/context coverage beyond the initial CoinDesk/Cointelegraph/Investing set.
- Changed: added additional public RSS candidates for Decrypt, CryptoSlate, Bitcoin Magazine, NewsBTC, CryptoPotato, SEC press releases, and CFTC press releases; expanded scheduled news symbol coverage to include SEI, OP, NEAR, FIL, RENDER, AVAX, and LINK.
- Check: `node --check tools/tradelab_news_impact.js` and `npm.cmd run tradelab:doctor` passed.
- Network run: expanded `tradelab:news` processed 190 feed items and matched 118 symbol events, then `tradelab:dependencies` produced updated dependency hypotheses.
- Result: broader source coverage shifted strongest dependency findings toward ETH, NEAR, XRP, and BTC downside reactions, with larger sample size than the original feed set.
- Safety conclusion: expanded news coverage remains research-only and does not affect real-money gate approval.

### 2026-06-06 - TradeLab drawdown diagnostics

- Goal: explain why the paper portfolio is losing before changing strategy rules or considering real money.
- Changed: added `tools/tradelab_drawdown_diagnostics.js`, `npm.cmd run tradelab:drawdown`, `TRADELAB_DRAWDOWN_DIAGNOSTICS.md`, and `tradelab-drawdown-diagnostics.json`; added the diagnostic to `tradelab:doctor`.
- Method: group paper losses by symbol, timeframe, strategy, and exit reason; list worst candidates; cross-check weak symbols against current news dependency hypotheses.
- Safety conclusion: drawdown diagnostics are triage-only and cannot clear the real-money gate.

### 2026-06-07 - TradeLab automatic quarantine

- Goal: stop repeatedly updating or auto-adding paper strategies that drawdown diagnostics already identified as weak.
- Changed: added `tools/tradelab_quarantine.js`, `npm.cmd run tradelab:quarantine`, `TRADELAB_QUARANTINE.md`, and `tradelab-quarantine.json`; wired quarantine into incubation, discovery, review cycle, report, and doctor checks.
- Method: block symbols with forward PnL <= -1000, exact candidates with forward PnL <= -500 or blocked health, and strategies with portfolio PnL <= -3000 plus high rejected ratio. Timeframes can be downranked for operator visibility.
- Safety conclusion: quarantine reduces paper churn and does not allow real-money trading; the gate remains authoritative.

### 2026-06-08 - TradeLab replacement scout

- Goal: search for reserve candidates to replace quarantined ideas without automatically adding risk while the kill-switch is active.
- Changed: added `tools/tradelab_replacement_scout.js`, `npm.cmd run tradelab:scout`, `TRADELAB_REPLACEMENT_SCOUT.md`, and `tradelab-replacement-scout.json`; added scout syntax coverage to `tradelab:doctor`.
- Method: scan non-quarantined symbols on 4h/1d using `sma-rsi` and `breakout`, require stable walk-forward, PF >= 1.65, DD <= 5.25%, health score >= 78, and no known/incubated duplicate key.
- Safety conclusion: scout output is inactive reserve only; it does not bypass quarantine, kill-switch, incubation, or the real-money gate.

### 2026-06-08 - TradeLab live candidate scoreboard

- Goal: make the remaining live paper candidates easy to monitor after quarantine reduced the active set.
- Changed: added `tools/tradelab_scoreboard.js`, `npm.cmd run tradelab:scoreboard`, `TRADELAB_SCOREBOARD.md`, and `tradelab-scoreboard.json`; wired scoreboard generation into the hourly review cycle and doctor.
- Method: score live candidates against manual-review requirements: observations, forward trades, PF, drawdown, loss streak, health, status, and gate blockers; label trend as collecting, improving, deteriorating, watch, quarantined, or rejected.
- Safety conclusion: scoreboard is visibility-only and does not change gate, quarantine, or real-money permissions.

### 2026-06-08 - TradeLab fetch retry and partial cycle

- Goal: make scheduled paper monitoring resilient to transient `fetch failed` network/API errors.
- Changed: added retry/backoff to `fetchCandles`, changed incubation to partial-cycle mode with per-candidate network errors, added `tools/tradelab_network_health.js`, `npm.cmd run tradelab:network`, `TRADELAB_NETWORK_HEALTH.md`, and `tradelab-network-health.json`; wired network health into cycle and doctor.
- Method: candle fetch retries 3 times with increasing delay; if a candidate still fails, its previous record is kept and the error is recorded instead of aborting the whole cycle.
- Safety conclusion: network resilience improves paper monitoring reliability only; it does not change strategy approval or real-money permissions.

### 2026-06-09 - TradeLab probation and promotion queue

- Goal: manage the lifecycle of live candidates instead of passively waiting for them to improve or fail.
- Changed: added `tools/tradelab_lifecycle.js`, `npm.cmd run tradelab:lifecycle`, `TRADELAB_LIFECYCLE.md`, and `tradelab-promotion-queue.json`; wired lifecycle into the review cycle and doctor.
- Method: deteriorating live candidates move to `probation`; repeated or severe probation can quarantine them; positive candidates with enough paper evidence enter a paper-only promotion queue for manual review.

### 2026-06-09 - TradeLab lifecycle idempotency guard

- Goal: prevent manual report refreshes from accidentally advancing probation cycles against unchanged market/incubation data.
- Changed: `tools/tradelab_lifecycle.js` now records the evaluated incubation `state.updatedAt` snapshot and does not increment probation cycles or repeat quarantine decisions when rerun on the same snapshot.
- Documentation: updated `TRADELAB.md` and `TASKS.md`.
- Check: `node --check tools/tradelab_lifecycle.js`, `npm.cmd run tradelab:doctor`, and two consecutive `npm.cmd run tradelab:lifecycle` runs passed; the second lifecycle run reported no status changes for the same input snapshot.
- Safety conclusion: promotion queue is not real-money approval and does not bypass gate, kill-switch, quarantine, or manual review.

### 2026-06-11 - TradeLab recovery sandbox

- Goal: keep replacement research moving while the portfolio kill-switch blocks new auto-incubation.
- Changed: daily discovery now writes `tradelab-recovery-sandbox.json` with strict paper recovery ideas and profitable near misses; `TRADELAB_DAILY_DISCOVERY.md` includes a Recovery Sandbox section.
- Check: `node --check tools/tradelab_daily_discovery.js`, `npm.cmd run tradelab:doctor`, and `npm.cmd run tradelab:discover` passed. Latest discovery tested 225 rows, found 1 strict recovery row and 12 near misses, and added 0 auto-incubation rows while kill-switch remained active.
- Safety conclusion: recovery sandbox rows are paper-only research and do not bypass quarantine, kill-switch, incubation, manual review, or the real-money gate.

### 2026-06-11 - TradeLab recovery promotion

- Goal: move one strict recovery sandbox idea into cautious paper incubation without opening real-money trading.
- Changed: added `tools/tradelab_promote_recovery.js`, `npm.cmd run tradelab:recovery:promote`, `TRADELAB_RECOVERY_PROMOTION.md`, and syntax coverage in `tradelab:doctor`.
- Changed: incubation now preserves previous `quarantined` status and recovery metadata across refreshes, preventing lifecycle-quarantined rows from reappearing as live candidates during later cycles.
- Check: `node --check tools/tradelab_promote_recovery.js`, `node --check tools/tradelab_incubate_once.js`, `npm.cmd run tradelab:doctor`, `npm.cmd run tradelab:recovery:promote`, and `npm.cmd run tradelab:cycle` passed.
- Result: promoted `ATOMUSDT:1h:sma-rsi` from recovery sandbox to paper auto-incubation. Scoreboard now has 2 live rows: `SEIUSDT:1h:sma-rsi` improving and `ATOMUSDT:1h:sma-rsi` collecting.
- Safety conclusion: the real-money gate remains `BLOCKED`; promotion is paper-only and still requires incubation, lifecycle, quarantine, kill-switch clearance, manual review, and explicit future real-money implementation.
...
### 2026-06-06 - Candle site dropdown category routing

- Goal: make dropdown menus behave like proper navigation for the candle site.
- Changed: dropdown parent links now open the overview pages (`wholesale.html`, `maker-goods.html`), submenu items open `category.html?section=...&item=...`, and `main.js` renders the selected category with a calculator link.
- Changed: dropdowns close on outside click, Escape, and when another dropdown opens.
- Fix: restored Russian text encoding in the existing candle pages and `main.js` after a bad PowerShell rewrite exposed mojibake.
- Check: `node --check main.js`, `npm.cmd run qa`, and Puppeteer browser smoke passed. Browser smoke confirmed outside-click close, Escape close, parent href, submenu href, and category title/calculator URL.
- Result: screenshot saved as `candle-dropdown-category-check.png`.

### 2026-06-07 - Candle category marketplace cards

- Goal: make each submenu category page look like a product listing rather than an information page.
- Changed: `category.html` now renders only a category heading plus a product-card grid; `main.js` generates 10 sample products per selected category.
- Changed: category cards were restyled toward a Wildberries-like marketplace format: compact width, tall photo area, price, title, short text, and calculate button.
- Check: `node --check main.js`, `npm.cmd run qa`, and Puppeteer desktop/mobile smoke passed. Browser smoke confirmed 10 cards, 10 photo blocks, calculator links, two mobile columns, and no horizontal overflow.
- Result: screenshots saved as `candle-category-wb-desktop.png`, `candle-category-wb-mobile.png`, and `candle-category-wb-mobile-final.png`.

### 2026-06-07 - Candle site visual and content harmonization

- Goal: bring all candle-site pages into one visual system after the structure was settled.
- Changed: rebuilt `styles.css` around a unified red/wine/blue/blush palette, removed obsolete promo/admin/old-hero styles, and added Google fonts `Manrope` plus `Cormorant Garamond`.
- Changed: cleaned `main.js` by removing unused admin/product-grid code and old scent data, then polished category-card copy.
- Changed: improved copy on the home page, calculator, wholesale/maker section pages, partners, contacts, and footers.
- Check: `node --check main.js`, `npm.cmd run qa`, and Puppeteer smoke across 9 pages passed. Browser smoke confirmed no promo bars, no horizontal overflow, 12/8 section directories, 10 category products, and 6 reviews.
- Result: screenshots saved as `candle-harmonized-home.png`, `candle-harmonized-wholesale.png`, `candle-site-harmonized-desktop.png`, `candle-site-harmonized-mobile.png`, and `candle-harmonized-home-mobile.png`.

### 2026-06-07 - Candle site footer cleanup

- Goal: remove remaining lower black footer bars and make partners/contacts feel like normal site sections.
- Changed: removed `.footer` blocks from internal candle pages, added a footer-like contact strip inside the home hero, refreshed partner copy, and rebuilt contacts as a light full-height section.
- Check: `node --check main.js`, `npm.cmd run qa`, and Puppeteer smoke for home/partners/contacts passed. Browser smoke confirmed 0 `.footer` blocks, no promo bars, no horizontal overflow, and the home contact strip exists.
- Result: screenshots saved as `candle-index-footer-check.png`, `candle-partners-footer-check.png`, `candle-contacts-footer-check.png`, and `candle-index-footer-check-mobile.png`.

### 2026-06-07 - Candle partners page polish

- Goal: make the partners page look like a complete commercial section instead of a short placeholder.
- Changed: rebuilt `partners.html` with an intro block, startup checklist, four partnership formats, three-step process, and contact CTA.
- Changed: added responsive partner-specific styles for desktop, tablet, and mobile layouts.
- Check: `node --check main.js`, `npm.cmd run qa`, and Puppeteer desktop/mobile smoke passed. Browser smoke confirmed 4 partner cards, 3 process cards, no header CTA button, and no horizontal overflow.
- Result: screenshots saved as `candle-partners-polished-desktop.png` and `candle-partners-polished-mobile.png`.

### 2026-06-09 - Svetlo cart page

- Goal: add a cart section where users can form an order from product cards.
- Changed: added `cart.html`, navigation links to the cart, product-card "В корзину" buttons, localStorage cart persistence, quantity editing, remove/clear actions, and Telegram/WhatsApp/Email order links.
- Changed: added responsive cart layout styles and synchronized the implementation into `svechy-publish`.
- Check: `node --check main.js`, `npm.cmd run qa` in both root and `svechy-publish`, and Puppeteer cart smoke passed. Browser smoke confirmed 10 add-to-cart buttons, 1 added cart item, calculated total, generated messenger links, and no horizontal overflow on desktop/mobile.
- Result: screenshots saved as `candle-cart-desktop.png` and `candle-cart-mobile.png`.

### 2026-06-07 - AI implementation business project package

- Goal: create a new business project for implementing AI in companies.
- Changed: added `ai-business-project/` with a static presentation page, business plan, implementation playbook, sales playbook, governance notes, and 12-month financial model.
- Research: used current market framing from McKinsey, IBM, and Deloitte around the gap between AI experiments and scaled business value.
- Check: `node --check ai-business-project/app.js`, `npm.cmd run qa`, and `npm.cmd run check:tools` passed. Puppeteer visual smoke for `ai-business-project/index.html` passed with no console errors and no horizontal overflow at 1440px and 390px widths.
- Result: screenshots saved as `ai-business-project/preview-desktop.png` and `ai-business-project/preview-mobile.png`.

### 2026-06-07 - AI Research Analyst MVP

- Goal: turn the user's chosen pilot, internet information collection and analysis, into a usable AI-business demo.
- Changed: added `ai-business-project/research-assistant.html`, `research-assistant.css`, `research-assistant.js`, and `docs/RESEARCH_ANALYST_PLAYBOOK.md`.
- Behavior: the page creates search-query plans, manages source cards, scores source reliability, and generates findings, risks, recommendations, and quality score.
- Check: `node --check ai-business-project/research-assistant.js`, `npm.cmd run qa`, and `npm.cmd run check:tools` passed. Puppeteer smoke passed with 5 query cards, 3 source cards, no console errors, and no horizontal overflow at 1440px and 390px widths.
- Result: screenshots saved as `ai-business-project/research-assistant-desktop.png` and `ai-business-project/research-assistant-mobile.png`.

### 2026-06-07 - AI Research Analyst live internet prototype

- Goal: make the research analyst collect and analyze real internet sources, not only manual/demo cards.
- Changed: added `ai-business-project/research-server.js` with static serving, `/api/research`, `/api/analyze-urls`, DuckDuckGo HTML search parsing, page fetching, HTML cleanup, summary extraction, source typing, and reliability scoring.
- Changed: updated `research-assistant.html/js/css` with live-search controls, URL analysis, server status, fetched dates, and real source replacement.
- Check: `node --check ai-business-project/research-server.js`, `node --check ai-business-project/research-assistant.js`, `npm.cmd run qa`, and `npm.cmd run check:tools` passed.
- Check: live API test found candidates, fetched pages, and returned analyzed source cards. Puppeteer smoke on `http://127.0.0.1:5192/research-assistant.html` passed with no console errors and no horizontal overflow.
- Note: current search uses HTML search parsing without paid API keys, so results can vary by search engine markup and network access. A production version should use a stable Search API key.

### 2026-06-08 - AI Monitor Studio market packaging

- Goal: make the internet-monitoring pilot presentable and sellable, not just technically working.
- Changed: added `ai-business-project/ai-monitoring-offer.html` and `ai-monitoring-offer.css` as a market-facing product page.
- Changed: added `COMMERCIAL_PROPOSAL_AI_MONITORING.md`, `DEMO_REPORT_AI_MONITORING.md`, `SALES_SCRIPT_AI_MONITORING.md`, `TARGET_CLIENTS_AI_MONITORING.md`, and `PRICING_AI_MONITORING.md`.
- Positioning: sell `AI-мониторинг ресурсов под ключ` with a 7-10 day pilot, clear deliverables, and monthly support bundled over API/server costs.
- Check: `node --check` for app/research scripts, `npm.cmd run qa`, and Puppeteer desktop/mobile smoke for the offer page passed with no console errors and no horizontal overflow.
- Result: screenshots saved as `ai-business-project/ai-monitoring-offer-desktop.png` and `ai-business-project/ai-monitoring-offer-mobile.png`.

### 2026-06-08 - First sale readiness pack

- Goal: make the offer ready for outreach and a real first client meeting.
- Changed: updated commercial proposal and pricing around `AI-мониторинг + мини-база знаний`: 70 000 ₽ pilot, 15 000 ₽/month support, mini knowledge base up to 10 materials as first-client bonus.
- Added: `OBJECTIONS_AI_MONITORING.md`, `MEETING_DEMO_SCENARIO.md`, `CLIENT_BRIEF_AI_MONITORING.md`, and `GO_TO_MARKET_AI_MONITORING.md`.
- Channel plan: start with a mini-site, Avito service listing, Telegram/VK proof posts, and direct outreach to 50 niche leads; use VC/TenChat/Habr later after first cases.
- Check: `node --check` for app/research scripts, `npm.cmd run qa`, and Puppeteer desktop/mobile smoke for the updated offer page passed with no console errors and no horizontal overflow.

### 2026-06-08 - Broader AI service catalog

- Goal: clarify that the project is not limited to one monitoring task.
- Changed: added `service-catalog.html` and `docs/SERVICE_CATALOG_AI_PACKAGES.md`.
- Packages: AI monitoring, AI knowledge base, AI sales, AI documents, AI support, and AI management reports.
- Sales logic: lead with one concrete offer, then expand into adjacent packages after trust and first ROI.
- Check: `node --check` for scripts, `npm.cmd run qa`, and Puppeteer desktop/mobile smoke for `service-catalog.html` passed with 6 cards, no console errors, and no horizontal overflow.

### 2026-06-08 - First outreach pack for e-commerce

- Goal: prepare concrete outreach materials for the first niche instead of generic positioning.
- Chosen niche: marketing/e-commerce, because monitoring competitors, prices, promotions, new products, and reviews is easy to explain.
- Added: `FIRST_OUTREACH_ECOMMERCE.md`, `ECOMMERCE_DEMO_SCENARIO.md`, `CONTENT_PLAN_ECOMMERCE.md`, and `LEADS_TEMPLATE_ECOMMERCE.csv`.
- Updated: `GO_TO_MARKET_AI_MONITORING.md` and `README.md` to link the e-commerce outreach pack.
- Check: `node --check` for scripts and `npm.cmd run qa` passed.

### 2026-06-08 - Browser outreach dashboard

- Goal: turn the outreach pack into an operational workspace for the first sales sprint.
- Added: `outreach-dashboard.html`, `outreach-dashboard.css`, and `outreach-dashboard.js`.
- Behavior: 9 copyable outreach texts, 50 lead rows, localStorage status/notes, add lead, reset template, and CSV export.
- Check: `node --check ai-business-project/outreach-dashboard.js`, `npm.cmd run qa`, and Puppeteer desktop/mobile smoke passed with 9 message cards, 50 lead rows, no console errors, and no horizontal overflow.
- Result: screenshots saved as `ai-business-project/outreach-dashboard-desktop.png` and `ai-business-project/outreach-dashboard-mobile.png`.

### 2026-06-08 - Full company launch blueprint

- Goal: convert the AI Monitor Studio work into a coherent company launch project.
- Added: `company-launch-map.html`, `company-launch-map.css`, `COMPANY_LAUNCH_BLUEPRINT.md`, `ROADMAP_180_DAYS.md`, `USER_TASKS_NEXT_14_DAYS.md`, `BUDGET_AND_RESOURCES.md`, `RESOURCE_LINKS.md`, and `MASTER_CHECKLIST.md`.
- Research: checked current public pricing/resources for OpenAI API, Brave Search API, .ru domain registration, ЮKassa, and NPD/self-employed tax references.
- Check: `node --check` for active JS files, `npm.cmd run qa`, and Puppeteer desktop/mobile smoke for `company-launch-map.html` passed with 4 phases, no console errors, and no horizontal overflow.
- Result: screenshots saved as `ai-business-project/company-launch-map-desktop.png` and `ai-business-project/company-launch-map-mobile.png`.
