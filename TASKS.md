...
- [x] Update first-sale offer with mini knowledge-base bonus, objection handling, meeting demo scenario, client brief, and go-to-market channel plan.
- [x] Add broader AI service catalog showing packages for monitoring, knowledge base, sales, documents, support, and management reports.
- [x] Build first outreach pack for marketing/e-commerce: Avito listings, Telegram/VK posts, direct messages, demo scenario, content plan, and 50-lead CSV template.
- [x] Build `outreach-dashboard.html` as a browser sales workspace with copyable messages, 50 lead rows, local status tracking, and CSV export.
- [x] Build full company launch blueprint: visual launch map, 180-day roadmap, master checklist, 14-day user task list, budget/resources, and resource links.
- [x] Build `ai-business-project/sales-site/` as a clean public sales site for AI Monitor Studio with offer, process, pricing, FAQ, and lead form.
- [x] Build the next client-facing product surface: a cleaner deployable monitoring app version separate from the demo and internal sales dashboard.
- [ ] Connect the client monitoring app to the live `research-server.js` API or a paid search/API stack after the first real client brief.
...
- [x] Replace local canvas-generated product visuals with real product photography when brand assets are available.
...
- [x] Add replacement scout for inactive reserve candidates outside quarantine.
- [x] Add live candidate scoreboard for progress toward manual review.
- [x] Add fetch retry, partial paper cycle, and network health report.
- [x] Add probation and promotion queue lifecycle for live candidates.
- [x] Make TradeLab lifecycle idempotent for repeated manual runs on the same incubation snapshot.
- [x] Add recovery sandbox so discovery can keep researching replacements while portfolio kill-switch is active.
- [x] Add manual recovery promotion command for moving strict sandbox rows into paper auto-incubation.
...
- [x] Casino mirror: собрать коммерческий пакет для покупателя с понятной структурой себестоимости, работ и рекомендованной цены.
